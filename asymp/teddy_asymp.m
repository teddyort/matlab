function teddy_asymp(sys,wlow,whigh)
...
% Edited by Teddy to allow magnitude plots with phase
...
h=findall(get(gcf,'Children'),'Type','Axes');
h=h(2);
axes(h);
yxlim=axis;
hold on;
...
if strcmp(opts.MagUnits,'abs'); mag = 10.^(mag/20); end
...
if strcmp(opts.MagUnits,'abs'); magpoints = 10.^(magpoints/20); end
